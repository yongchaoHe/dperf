...
do_tcp(struct conn_t* conn, struct task_t* task, struct conn_client_t* ssc) {
    ...
    for (;;) {
        ...
        if (nb_rx > 0) {
            ...
            if (unlikely(acked_bytes >= task->len)) {
                break;
            }
        }
        ...
        while (burst_num < CLIENT_SIZE_BURST_TX) {
            ...
            // if (conn->client_id == 3 && (sent_pkts < 10))
            //     print_mraw(bufs_tx[burst_num]);
            burst_num++;
        }
        send_all(conn->port_id, conn->queue_id, bufs_tx, burst_num);

        counter++;
        if (unlikely(counter == 1000)) {
            if (*force_quit == true) {
                break;
            }
            counter = 0;
...
lcore_server(void* arg) {
    ...
    uint16_t tx_burst = SERVER_SIZE_BURST_TX;
